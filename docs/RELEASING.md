...
2. **Run it again with `dry-run` off.**
   - `version` — leave empty to auto-detect. git-cliff computes the next version
     ...
     counts, whatever the subject says. The `release bump matches title` PR
     check (AGENTS.md, "the commit" is two pieces of text) catches this before
     merge now — this is the fallback for anything that reached `main` before
     the check existed, or past it.
