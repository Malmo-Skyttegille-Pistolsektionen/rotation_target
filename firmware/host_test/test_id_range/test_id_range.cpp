...
void test_skips_ids_already_in_use() {
  ...
  const int32_t id =
      rt::next_free_id(100, [&](int32_t candidate) { return used.count(candidate) > 0; });
...
void test_an_upload_over_a_shipped_entry_shadows_it() {
  TEST_ASSERT_TRUE(
      rt::would_shadow_shipped(/*existing_readonly=*/true, /*incoming_readonly=*/false));
}
...
void test_a_shipped_entry_loaded_first_is_not_a_shadow() {
  ...
  TEST_ASSERT_FALSE(
      rt::would_shadow_shipped(/*existing_readonly=*/false, /*incoming_readonly=*/false));
  TEST_ASSERT_FALSE(
      rt::would_shadow_shipped(/*existing_readonly=*/false, /*incoming_readonly=*/true));
}
...
void test_two_shipped_entries_are_not_a_shadow() {
  ...
  TEST_ASSERT_FALSE(
      rt::would_shadow_shipped(/*existing_readonly=*/true, /*incoming_readonly=*/true));
}
