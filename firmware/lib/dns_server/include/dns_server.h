 ...
 *
 * @note Please use string literals (or ensure they are valid during dns_server lifetime) as names,
 * since we don't take copies of the config values `name` and `if_key`
 */
typedef struct dns_entry_pair {
  const char *name;   /**<! Exact match of the name field of the DNS query to answer */
  const char *if_key; /**<! Use this network interface IP to answer, only if NULL, use the static IP
                         below */
 ...
 *
 * @note If you want to define more rules, you can set `DNS_SERVER_MAX_ITEMS` before including this
 * header Example of using 2 entries with constant IP addresses
 * \code{.c}
 ...
 * based on configured rules, pairs of name and either IPv4 address or a netif ID (to respond by
 * it's IPv4 add)
 *
